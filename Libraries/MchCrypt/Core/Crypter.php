...
final class MchCrypt_Core_Crypter
{
    ...
    public function __construct($cipherId = self::CIPHER_BLOWFISH, $encryptionModeId = self::MODE_CBC) 
    {
		...
		if(!$this->isValidCipherId($cipherId) || (null === $this->getCipherKeySize()))
		{
			return null;
		}
    ...
    private function getCipherBlockSize()
    {
		...
		$arrSize[self::CIPHER_DES]          = 8;
		$arrSize[self::CIPHER_RC2]          = 8;
		$arrSize[self::CIPHER_GOST]         = 8;
		$arrSize[self::CIPHER_CAST_128]     = 8;
		$arrSize[self::CIPHER_CAST_256]     = 16;
		$arrSize[self::CIPHER_BLOWFISH]     = 8;
		$arrSize[self::CIPHER_RIJNDAEL_128] = 16;
		$arrSize[self::CIPHER_RIJNDAEL_192] = 24;
		$arrSize[self::CIPHER_RIJNDAEL_256] = 32;
